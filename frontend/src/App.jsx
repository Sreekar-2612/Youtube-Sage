...
export default function App() {
    ...
    useEffect(() => {
        if (isPrivate) {
            setMessages([]);
            if (!session) {
                api.loadVideo("temp-chat", provider, modelName)
                    .then(result => {
                        setSession(result);
                    ...
                    .catch(e => setError("Failed to initialize private chat: " + e.message));
            }
        } else {
            if (session) {
                if (session.video_id === "temp-chat") {
                    setSession(null);
                    setMessages([]);
                } else {
                    setMessages(sessionMessages[session.session_id] || []);
                }
            } else {
                setMessages([]);
            }
    ...
    async function handleLoad() {
        ...
        try {
            ...
            // Add to stack history (most recent on top) if not private
            if (!isPrivate && result.video_id !== "temp-chat") {
    ...
    function handleSelectSession(histItem) {
        ...
        if (histItem.video_id === "temp-chat") {
            setVideoUrl("");
        } else {
                        ...
                        <div className="space-y-4 overflow-y-auto max-h-[500px] pr-1">
                            {history.map((item, index) => (
                                <div
                                    ...
                                    className={`p-4 rounded border cursor-pointer transition-all duration-200 ${session?.session_id === item.session_id
        ...
        return (
            <div className="w-screen h-screen flex items-center justify-center bg-background relative overflow-hidden select-none">
                ...
                <div className="bg-white tech-border border-t-8 border-t-primary p-10 max-w-md w-full shadow-2xl rounded text-center z-10">
                    <h1 className="font-display-lg text-primary tracking-wider uppercase mb-2 flex items-baseline justify-center">
                        <span className="text-3xl font-semibold normal-case" style={{ fontFamily: "'Brush Script MT', 'Brush Script SDK', cursive" }}>You</span>
                        <span className="text-4xl font-black ml-0.5">Yap</span>
                    </h1>
                ...
                <div className="mt-auto p-4 flex flex-col gap-4">
                    ...
                    <div className="flex flex-col gap-2 border-t border-outline-variant pt-4">
                        ...
                        >
                            ...
                            {pingStatus !== "Idle" && (
                                <span className={`ml-auto text-[8px] font-bold px-1.5 py-0.5 rounded border uppercase ${pingStatus.startsWith("Online")
                ...
                <header className="flex justify-between items-center px-8 w-full h-16 border-b border-outline-variant bg-surface z-40">
                    ...
                    <div className="flex items-center gap-6">
                        <button 
                            ...
                            style={isPrivate ? { fontVariationSettings: "'FILL' 1" } : {}}
                            title={isPrivate ? "Private Chat Mode: ON" : "Private Chat Mode: OFF"}
                        >
